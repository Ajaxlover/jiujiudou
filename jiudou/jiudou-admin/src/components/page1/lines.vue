<template>
  <div class="lines">
    <div class="switchButton">
      <a :class="{ active: value == 1 }" @click="handleSelect(1)">月</a
      ><a :class="{ active: value == 2 }" @click="handleSelect(2)">日</a>
    </div>
    <div id="bottom_1"></div>
  </div>
</template>
    ...
    setChart() {
      let option = {
        ...
        tooltip: {
          ...
          formatter: params => {
            let seriesId = params.seriesId.substr(
              params.seriesId.length - 1,
              1
            );
            if (seriesId == 0) {
              return (
