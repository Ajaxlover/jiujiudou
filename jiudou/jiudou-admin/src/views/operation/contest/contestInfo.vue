  ...
  ><div class="contest-sm-edit">
    <el-container>
      ...
      <el-main>
          ...
          <div class="bottom-part-l">
            ...
            <div class="bottom-f">
              ...
              >
                ...
                <el-form-item label="邀请函" prop="contestInviteContent">
                  <Ueditor
                    v-model.trim="form.contestInviteContent"
                ...
                <el-form-item label="获奖名单">
                  ...
                  <el-upload
                    ...
                    :show-file-list="false"
                    :auto-upload="true"
                    :http-request="uploadNameListPic"
                    :before-upload="beforeAvatarUpload"
                  >
                    <img
                      v-if="form.nameListPic"
                      :src="form.nameListPic"
                      class="avatar"
                    />
                    <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                  </el-upload>
                  <el-button
                    @click="viewImg(form.nameListPic)"
                    v-if="form.nameListPic"
                    type="text"
                    size="small"
                    >预览</el-button
        ...
        <div class="save-btn" style="margin-top:130px">
          <div>
            <!-- <el-button size="small" @click="doCreateContest" type="primary"
              >创建</el-button
            > -->
            <el-button size="small" @click="doSave" type="primary"
              >确定</el-button
            >
          </div>
        </div>
    ...
    >
      ...
      <template>
        <el-form
          ...
          style="margin:0 90px;"
        >
          <!-- <el-form-item label="标题" prop="contestInviteTitle">
            <el-input
              v-model.trim="form.contestReadingTitle"
              size="small"
            ></el-input>
          </el-form-item> -->
          <el-form-item label="内容" prop="contestInviteContent">
            <Ueditor
              v-model.trim="form.contestInviteContent"
              :initialFrameHeight="initialFrameHeight"
            />
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        honorPicFile: null,
        nameListPic: "",
        nameListPicFile: null,
        price: "0",
        isNotFree: false,
        contestInviteTitle: "", // 报名须知标题
        contestInviteContent: "", // 报名须知正文
        coverPFile: "", // 竞赛首页图片
        examTime: "" // 竞赛报名时间
      },
  ...
  methods: {
    ...
    // 上传入围名单
    uploadNameListPic(params) {
      this.form.nameListPic = params;
      axios
        .request({
          method: "post",
          url: `/exam/masterHead/getUploadCosPath`
          ...
          if (ret.code === 200) {
            ...
            axios
              ...
              .then(res => {
                if (res.status === 200) {
                  let { accessUrl } = res.data;
                  this.form.nameListPic = accessUrl;
                } else {
                  this.$message({
                    type: "warning",
                    message: "获奖名单上传失败"
    ...
    onChange(file, fileList) {
      ...
      reader.onload = event => {
        console.log(event);
        this.form.honorPic = event.target.result;
      };
      reader.readAsDataURL(file.raw);
      this.form.honorPicFile = file.raw;
    },
    fileChange(file, fileList) {
      ...
      reader.onload = event => {
        console.log(event);
        this.form.nameListPic = event.target.result;
      };
      reader.readAsDataURL(file.raw);
      this.form.nameListPicFile = file.raw;
    },
    handleClose() {
    ...
    getContestInfo() {
      axios
        .request({
          method: "post",
          url: "/exam/masterHeadCompete/findById",
          data: {
            id: this.id
          }
        })
        .then(res => {
          ...
          if (ret.code === 200) {
            ...
            this.form.isNotFree = isFee == 1;
            this.form.examTime = [startTime, endTime];
            this.form.contestInviteContent = notes;
            this.form.nameListPic = coverUrl;
            this.form.honorPic = certificateTemp;
          } else {
            this.$message({
              type: "warning",
              message: ret.msg
    ...
    doSave() {
      let {
        ...
        isNotFree,
        contestInviteTitle,
        contestInviteContent,
        coverPFile,
        examTime
      } = this.form;
      let fd = new FormData();
      fd.append("id", this.id);
      fd.append("masterheadId", this.masterheadId);
      fd.append("notes", contestInviteContent || "");
      ...
      // fd.append("certificateTemp", honorPic || "");
      fd.append("coverUrl", nameListPic || "");
      ...
      this.$refs["contestForm"].validate(valid => {
        if (valid) {
          axios
            .request({
              method: "post",
              url: "/exam/masterHeadCompete/edit",
              data: fd
            })
            .then(res => {
              let ret = res.data;
