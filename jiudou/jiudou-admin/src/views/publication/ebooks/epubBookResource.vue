<template>
  <div class="ebookResource">
    <el-container class="bookResourceTop">
      <el-main v-if="loading.initLoading">
        <div class="main-resource"><iframe id="epubEditor" :src="iframeUrl" style="width:100%;height:750px;border:0"></iframe></div>
      </el-main>
    </el-container>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    activeName: function(val) {
      if (val === "second" && !this.loading.initLoading) {
        // this.initCOS();
        // this.intVOD();
  ...
  methods: {
    ...
    jump() {
      this.$router.push({
        path: "/publication/epubBookBindResource",
        query: {
          id: this.id
        }
      });
    ...
    handleSearchRes() {
      ...
      this.$router.push({
        path: "/publication/ebookSearchRes",
        query: {
          id: this.id,
    ...
    handleSearchTest(turnType) {
      this.$router.push({
        path: "/publication/ebookSearchTest",
        query: {
          id: this.id,
          catalogid: this.currentCatalog.catalogid,
          turnType: turnType
    ...
    lookBookRes() {
      axios.request({
        ...
        let ret = res.data
        if (ret.code === 200) {
          this.basicForm = { id: 0, savetype: 'insert', resname: 'epub-0-' + this.id, page: 0 };
          var tbookResources = ret.data.tbookResources ? ret.data.tbookResources[0] : false;
          if (tbookResources) {
            this.basicForm.savetype = 'update';
            this.basicForm.resurl = tbookResources.resUrl;
            this.basicForm.id = tbookResources.id;
            this.basicForm.page = ret.data.page;
          }
          this.basicForm.resname = 'epub-0-' + this.id;
          this.basicForm.dtbookid = this.id;
          this.basicForm.token = this.$store.state.user.token;
          this.basicForm.uid = this.$store.state.user.userId;
          this.iframeUrl = '/epub/src/index.html?dtbookid=' + this.id +
            '&id=' + this.basicForm.id +
            '&savetype=' + this.basicForm.savetype +
            '&resname=' + this.basicForm.resname +
            '&token=' + this.basicForm.token +
            '&uid=' + this.basicForm.uid +
            '&page=' + this.basicForm.page +
            '&publishname=' + ret.data.publishname +
            '&coverpic=' + ret.data.coverPic +
            '&author=' + ret.data.author +
            '&tname=' + ret.data.tname +
            '&pubdate=' + ret.data.pubDate +
            '&tbNum=' + ret.data.tbNum +
            (this.basicForm.savetype == 'update' ? ('&resurl=' + this.basicForm.resurl) : '')
          // 将数据传给iframe
          // document.getElementById('epubEditor').contentWindow.postMessage(this.basicForm, '*')
        }

        this.loading.initLoading = true;
      })
    }
  }
}
