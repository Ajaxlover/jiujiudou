<template>
  <el-container class="book">
    ...
    <el-main v-loading="dataLoading.textbookData">
      <div v-show="textbookData.length > 0">
        <div class="top">
          <el-input
            size="small"
          ...
          >
            ...
            >
              <div slot-scope="scope" class="info">
                ...
                <div class="right">
                  <div class="name" @click="editBook(scope.row)">
                    {{ scope.row.tname }}
            ...
            <el-table-column label="参与编辑" align="center">
              <div slot-scope="scope" v-if="scope.row.joinEditor">
                {{ scope.row.joinEditor }}
              </div>
            </el-table-column>
            ...
            <el-table-column label="时间" align="center">
              <template slot-scope="scope">
                <div>创建时间:{{ timeFormat(scope.row.createTime) }}</div>
                <div>更新时间:{{ timeFormat(scope.row.updateTime) }}</div>
                <div v-if="scope.row.publishTime">
                  申请时间:{{ timeFormat(scope.row.publishTime) }}
                </div>
              </template>
            </el-table-column>
            <!-- <el-table-column label="出版规划" align="center" width="100">
            ...
            <el-table-column label="参与方式" align="center" width="100">
              <div slot-scope="scope">
                <span v-if="scope.row.joinType == 1">总编辑</span>
                <span v-else>辅助编辑</span>
              </div>
            </el-table-column>
                ...
                <!-- <el-dropdown trigger="hover" :hide-on-click="true">
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item @click.native="handleTransfer(scope.row)"
                      >转让</el-dropdown-item
                    >
                    <el-dropdown-item @click.native="copyDtBook(scope.row)"
                      >复制</el-dropdown-item
                    >
                    <el-dropdown-item @click.native="delBook(scope.row)"
                      >删除</el-dropdown-item
                    >
                  </el-dropdown-menu>
                </el-dropdown> -->
      ...
      <div v-show="textbookData.length == 0">
        ...
        <div
          style="height:600px;padding:0 100px;font-size:30px;color:#409eff;display:flex;align-items: center;"
        >
          <span v-if="!dataLoading.textbookData">
            尊敬的用户，您尚未创建数字教材。请点击页面右上角“+新建”按钮，立即创建数字教材，并体验新型数字教材编辑功能。
          </span>
        </div>
      </div>
    </el-main>
    ...
    >
      ...
      <span slot="footer">
        <el-button size="small" type="primary" @click="createBook"
          >创建</el-button
        >
    ...
    >
      ...
      <span style="padding-left:75px">选择转让编辑：</span>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      axios
        .request({
          method: "post",
          // url: "/ebook/digitalBook/list",
          url: "/ebook/digitalBook/queryMyEditorPage",
          data: params
        })
        .then(res => {
          this.loading = false;
    ...
    createBook() {
      ...
      axios
        .request({
          method: "post",
          url: "/ebook/digitalBook/addByTeacher",
          data
        })
        .then(res => {
          this.loading = false;
    ...
    clearAddForm() {
      ...
      this.addForm.num = "";
      this.addForm.author = "";
      this.addForm.pubName = "";
      this.addForm.subjectType = 16;
      this.addForm.pubPlan = "1";
      this.addForm.editType = "2";
    },
    copyDtBook(row) {
      // @todo
      this.$confirm("确定要复制本书吗？复制后可进行二次编辑", "提示", {
    ...
    editBook(row) {
      this.$router.push({
        path: "/teacher/teaDtBookInfo",
        query: {
          dtbookId: row.dtbookId
        }
      });
      // if (row.editType == 1) {
      //   this.$router.push({
  ...
  mounted() {
    this.dataLoading.textbookData = true;
    setTimeout(() => {
      this.dataLoading.textbookData = false;
    }, 500);
