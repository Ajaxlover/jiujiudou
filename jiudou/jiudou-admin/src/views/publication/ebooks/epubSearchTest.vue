<template>
  <el-container class="searchRes">
    <el-header height="50px">
      <span class="book" @click="toBook">绑定习题</span>
    ...
    <el-main>
      ...
      <el-form :inline="true" v-model="testSelectForm" size="small">
        ...
        <el-form-item label="难度系数">
          <el-select
            v-model="testSelectForm.difficultyLevel"
            style="width:140px"
            placeholder="难度系数"
            @change="testSearch"
          >
  ...
  methods: {
    ...
    loadSubjects() {
      ...
      let data = {
        ...
        difficultRateLevel: this.testSelectForm.difficultyLevel
      };
    ...
    changeSelect(select) {
      ...
      select.forEach(element => {
        ...
        obj.difficultyRate = element.difficultyRate;
    ...
    doBindBookTest() {
      ...
      //   return;
      // }
      let url = "/ebook/dTbookSubjectBank/bingSubject";
      // if (this.turnType == 3) {
      ...
      axios
        ...
        .then(res => {
          ...
          if (ret.code == 200) {
            var type = '';
            var uuid = '';
            ret.data.split("&").map(function (item) {
              if (item) {
                var res = item.split("=");
                if (res[0] == 'type') type = res[1];
                else if (res[0] == 'uuid') uuid = res[1];
              }
            });
            if (uuid && type) {
              this.$message({
              ...
              window.parent.postMessage([{
                ...
                resType: type,
                resName: '习题',
                resUrl: window.location.protocol + "//" + window.location.host + '/student/digital-staff?type=' + type + '&uuid=' + uuid
              }]);
              this.doReturn();
            } else {
              this.$message({
