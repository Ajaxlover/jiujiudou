  ...
  <el-container class="searchRes">
    ...
    <el-main>
      ...
      <el-form :inline="true" v-model="testSelectForm" size="small">
        ...
        <el-form-item v-if="testSelectForm.rlType == 2">
          <el-select
            v-model="testSelectForm.textBookId"
            size="small"
            style="width:120px;"
        ...
        <el-form-item v-if="testSelectForm.rlType == 2" label="章节">
          <el-tree-select
            @input="bookTipSelectChange"
            :styles="tipTreeStyle"
            :value="testSelectForm.catalogId"
            :selectParams="bookSelectParams"
            :treeParams="bookTreeParams"
            @searchFun="bookTipSearch"
            ref="treeSelect"
          />
        </el-form-item>
      ...
      <div class="bottomBtnBox">
        ...
        <el-button size="small" @click="doBindBookTest">确定</el-button>
      </div>
    </el-main>
    <!-- 预览 -->
    <el-dialog
...
export default {
  ...
  methods: {
    ...
    resSearchByBook() {
      ...
      this.getChapterByTextBook(this.testSelectForm.textBookId);
      this.loadSubjects();
    },
    // 根据教材获取章节
      ...
      axios
        ...
        .then(res => {
          ...
          if (ret.code === 200) {
            ...
            this.bookTreeParams.data = ret.data;
      ...
      axios
        ...
        .then(data => {
          ...
          if (res.code == 200) {
            ...
            this.testSelectForm.textBookId = res.data[0].textbookId
              ? res.data[0].textbookId
              : "";
            if (res.data.length > 0) {
              this.getChapterByTextBook(res.data[0].textbookId);
            }
    ...
    loadSubjects() {
      ...
      if (this.testSelectForm.rlType == 1 || this.testSelectForm.rlType == 2) {
        url = "/v1/subjectBank/getTKExercisesByPage";
        data.difficultRateLevel = this.testSelectForm.diff;
        data.textbookId = this.testSelectForm.textBookId;
        data.catalogId = this.testSelectForm.catalogId.join(",");
      ...
      if (this.testSelectForm.rlType == 2 && !this.testSelectForm.textBookId) {
        this.testSelectList = [];
        this.testSelectPage.total = 0;
        return;
      }
      axios
        .request({
    ...
    async slTypeChange(row) {
      this.testSelectForm.catalogId = [];
      this.testSelectForm.textBookId = "";
      if (row == 2) {
    ...
    doBindBookTest() {
      ...
      let url = "/v1/selfTest/addSbFromLibrary";
      if (this.turnType == 3) {
    ...
    bookTipSelectChange(ids, labels) {
      this.testSelectForm.catalogId = ids;
      this.testSelectPage.page = 1;
      this.loadSubjects();
    },
    // 知识点搜索
    ...
    doReturn() {
      this.$router.push({
        ...
        query: {
          textbookId: this.textbookId,
          returnType: this.turnType, //1是返回扫码资源 2是返回自测习题 3是返回教材习题
