    ...
    <el-main>
      <div class="contentBox">
        <div class="leftList">
          ...
          <div class="setScore">
            分数：{{ timuTotalScore ? timuTotalScore : 0 }}
            <el-button
              type="text"
          ...
          <div class="paperBtn">
            ...
            <el-button type="primary" size="small" @click="addSelectTimu"
              >添加题目</el-button
            >
            <!-- <el-button type="primary" size="small" @click="publishExercise"
              >发布</el-button
            > -->
          </div>
        </div>
...
import timu from "./task/timu";
import addTest from "./addTest_1";
...
export default {
  ...
  methods: {
    ...
    loadData(param) {
      ...
      axios
        ...
        .then(res => {
          let ret = res.data;
          if (ret.code === 200) {
            this.exerciseName = ret.data.exerciseName;
    ...
    publishExercise() {
      ...
      if (this.timuTotalScore != 100) {
        this.$message({
          type: "warning",
          message: "请设置作业总分为100分"
        });
        return;
      }
      if (!this.endTime) {
    ...
    setScoreCommon() {
      ...
      this.scoreListData.forEach(el => {
        el.children.forEach(element => {
          if (element.sonList.length > 0) {
            // 有组合题
            element.sonList.forEach(i => {
              let objSon = {};
              objSon.subjectId = i.subjectId;
              objSon.score = element.score / element.sonList.length;
      ...
      if (this.setScoreTotal != 100 && this.remindDialogShow) {
        ...
        axios
          .request({
            ...
            url: "/v1/ware/addExercise",
            data: {
              exerciseItemJson: JSON.stringify(this.setScoreArr),
              exerciseName: this.exerciseName,
              // wareId: this.coursewareId,
              exerciseId: this.exerciseId
                ? this.exerciseId
          ...
          .then(res => {
            ...
            if (ret.code == 200) {
              ...
              this.loadData(ret.data.exerciseId);
            } else {
              this.$message({
                type: "warning",
                message: ret.msg
    ...
    handleDel(i) {
      this.$confirm("<div>确定删除吗？</div>", "提示", {
        ...
        .then(() => {
          ...
          // 获取删除的题目ID
          let removeSubjectIds = [];
          if (i.tSubject == 11) {
            removeSubjectIds.push(i.id);
            if (i.sonList.length > 0) {
              i.sonList.forEach(a => {
                removeSubjectIds.push(a.subjectId);
              });
            }
          } else {
            removeSubjectIds.push(i.id);
          ...
          axios
            .request({
              ...
              data: {
                exerciseName: this.exerciseName,
                exerciseItemJson: JSON.stringify(this.jsonArr),
                removeSubjectIds: removeSubjectIds.join(","),
                exerciseId: this.exerciseId
              }
            })
            .then(res => {
