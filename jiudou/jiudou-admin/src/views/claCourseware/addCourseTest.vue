  ...
  <div class="addTestTask">
    <div>
      <div style="margin-bottom:40px">
        ...
        <el-radio-group @change="onRadiooChange" v-model="importRadio">
...
export default {
  ...
  mounted() {
    ...
    let sblCourseId = JSON.parse(sessionStorage.getItem("sblCourseId"));
    if (sblCourseId) {
      this.getTreeList(sblCourseId);
    }
  ...
  methods: {
    ...
    doSure() {
      ...
      this.$store.commit("pushLibHomeList", this.contentList);
      this.$emit("addSuccess", this.contentList);
      this.doReturn();
    },
    // 预览
        ...
        .then(res => {
          ...
          if (ret.code === 200) {
            ...
            if (arr && arr.length > 0) {
              ...
              this.testForm.sblLib[1] = this.testLibOptions[0].children[0].id;
            }
            this.loadTestList();
          } else {
            this.$message({
