  ...
  <div class="addTestTask">
    <div>
      <div style="margin-bottom:30px">
        <span style="margin-right:15px;padding-left:25px"
        ...
        <el-input
          ...
          style="width:900px"
          show-word-limit
          placeholder="章节编号直接输在标题内，如：第1章 反函数与反三角函数"
        ></el-input>
      </div>
      <div style="margin-bottom:40px">
        <span style="margin-right:15px">内容来源</span>
        <el-radio-group @change="labelChange" v-model="importRadio">
          <el-radio label="1" border>题库获取</el-radio>
          <el-radio label="2" border>教材题库获取</el-radio>
        ...
        >
          <div class="batchAdd-ts">
            题库获取：<span>从个人库或已购题库获取题目到当前云课</span>
          </div>
          <div class="batchAdd-ts">
            教材题库获取：<span>从已验证的教材中获取题目到当前云课</span>
          </div>
          <!-- <div class="batchAdd-ts">
            其他云课导入：<span>从公共云课、班级云课导入内容到当前云课</span>
          </div> -->
          <i
          ...
          <el-form :inline="true" size="small" :model="testForm">
            ...
            <el-form-item label="题型" style="margin-left:15px;">
              ...
              >
                ...
                <el-option label="组合" :value="11"></el-option>
              </el-select>
            </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      bookList: [], // 教材列表
      importRadio: "1",
      taskTitle: this.currentTestTaskTitle,
      contentList: [],
      testTag: [], //题库选择生成的数组
  ...
  methods: {
    ...
    clearSelectedTest() {
      ...
      if (this.$refs.testTable) {
        this.$refs.testTable.clearSelection();
      }
      if (this.$refs.cloudTestTable) {
        this.$refs.cloudTestTable.clearSelection();
      }
      if (this.$refs.bookTestTable) {
    ...
    doReturn() {
      this.$emit("toCloseDrawer2");
    },
    doSure() {
      if (this.taskTitle.split(" ").join("").length != 0) {
        if (this.contentList.length > 0) {
          this.loading.doSureLoading = true;
          if (this.currentTestTaskId) {
            // 修改
            axios
              .request({
                method: "post",
                url: "/ccourse/v4/cCourseCatalog/updateTestTask",
                data: {
                  testTaskId: this.currentTestTaskId,
                  sbsStr: JSON.stringify(this.contentList),
                  testTaskName: this.taskTitle
                }
              })
              .then(res => {
                let ret = res.data;
                ...
                if (ret.code === 200) {
                  ...
                  this.$emit("toCloseDrawer2LoadContent");
                }
              });
          } else {
            //添加
            axios
              .request({
                method: "post",
                url: "/ccourse/v4/cCourseCatalog/createTestTask",
                data: {
                  catalogCourseId: this.catalogCourseId,
                  catalogParentId: this.catalogParentId,
                  sbsStr: JSON.stringify(this.contentList),
                  catalogName: this.taskTitle
                }
              })
              .then(res => {
                let ret = res.data;
    ...
    // 难点
    formatDiff(row) {
      if (row.hasOwnProperty("sbDifficultyLevel")) {
        return common.diffFormat(row.sbDifficultyLevel);
      }
      if (row.hasOwnProperty("difficultyLevel")) {
