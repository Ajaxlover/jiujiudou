  ...
  <div class="chapterInfoBox">
    <el-container class="chapterInfo">
      <el-aside width="350px">
        <div
          style="background-color: #2ba1f4;"
          :class="[isCatalog ? 'listHeader1' : 'listHeader']"
        >
          <!-- @click="clickCourse" -->
          <span @click="clickCourse">{{ courseName }}</span>
          <img
            v-if="isCatalog"
          ...
          >
            <el-table-column label="课程内容列表" :render-header="renderHeader">
              <div slot-scope="scope">
                <span
                  v-if="scope.row.catalogSubType == 8"
                  style="cursor:pointer"
                  @click="showTimuDialog(scope.row)"
              ...
              <div slot-scope="scope">
                ...
                <el-dropdown trigger="click" :hide-on-click="false">
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item>
                      试看<el-switch
                        :value="scope.row.catalogIsFree == 1"
                        style="margin-left:30px;"
                        @change="tryView(scope.row)"
                      ></el-switch>
      ...
      <jdialog
        ...
        width="800px"
        :noFooter="true"
        :visible.sync="isShowTimuDialog"
      >
        <template slot="body">
          <div>
            <ul style="max-height:600px;overflow-y: auto">
              <li
                v-for="(item, index) in cloudCourseTimuList"
                :key="item.id"
                :id="item.id"
              >
    ...
    <el-container class="chapterInfoBtm">
      <el-card>
        ...
        <div class="materialBody" v-loading="loading.courseMatchLoading">
          <div v-if="sucaiPackList.length > 0">
            <div
              v-for="(item, idx) in sucaiPackList"
              :key="idx"
              style="font-size:14px;margin-top:10px;"
            >
              <span>{{ item.ccSupportMaterialName }}</span>
              <span style="margin-left:15px">{{
                materialSize(item.ccSupportMaterialSize)
              }}</span>
              <span style="margin:0 20px;">{{
                materialTime(item.ccSupportMaterialTime)
              }}</span>
              <i
                class="el-icon-close"
                style="cursor:pointer"
                @click="deleteMaterial(item.ccSupportMaterialTime)"
              ></i>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      sucaiPackList: [],
      cloudCourseTimuList: [], // 云课章节下所有题目
      isShowTimuDialog: false,
  ...
  methods: {
    // 云课章节获取云课题目
    getCloudCourseTimu(catalogId) {
      axios
        .request({
          method: "post",
          url: `${process.env.BASE_URL_V3}/cloudCourse/items`,
          data: {
            catalogId
          }
        })
        .then(res => {
          let ret = res.data;
          if (ret.code === 200) {
            this.cloudCourseTimuList = ret.data;
            this.isShowTimuDialog = true;
          }
        });
    },
    showTimuDialog(row) {
      const { catalogId } = row;
      this.getCloudCourseTimu(catalogId);
    },
    //判断节点能否被放置
    ...
    handleDrop(draggingNode, dropNode, dropType, ev) {
      ...
      let parentId = 0;
      if (dropType == "before") {
        parentId = dropNode.data.catalogParentId;
        postId = dropNode.data.catalogId;
        preId = 0;
      } else if (dropType == "after") {
        parentId = dropNode.data.catalogParentId;
        preId = dropNode.data.catalogId;
        postId = 0;
      } else {
        parentId = dropNode.data.catalogId;
        preId = 0;
        postId = 0;
      }
      ...
      axios
        .request({
          method: "post",
          url: "/ccourse/v4/cCourseCatalog/moveCatalog",
          data: {
      ...
      Sortable.create(tbody, {
        ...
        onEnd({ newIndex, oldIndex }) {
          ...
          if (newIndex > oldIndex) {
            preId = saveThis.contentList[newIndex].catalogId;
            postId = 0;
          } else if (newIndex < oldIndex) {
            preId = 0;
            postId = saveThis.contentList[newIndex].catalogId;
          } else {
            return;
          }
          axios
            .request({
              ...
              url: "/ccourse/v4/cCourseCatalog/moveCatalog",
              data: {
                catalogId: saveThis.contentList[oldIndex].catalogId,
                parentId: saveThis.isCatalog
                  ? saveThis.currentCatalog.catalogId
                  : 0,
                preId: preId,
                postId: postId
              }
    ...
    loadCourseDetail() {
      axios
        ...
        .then(res => {
          let ret = res.data;
          if (ret.code === 200) {
            let urlList = ret.data.ccSupportMaterialUrl
              ? ret.data.ccSupportMaterialUrl.split(",")
              : [];
            let ccSupportMaterialNameList = ret.data.ccSupportMaterialName
              ? ret.data.ccSupportMaterialName.split(",")
              : [];
            let ccSupportMaterialTimeList = ret.data.ccSupportMaterialTime
              ? ret.data.ccSupportMaterialTime.split(",")
              : [];
            let ccSupportMaterialSizeList = ret.data.ccSupportMaterialSize
              ? ret.data.ccSupportMaterialSize.split(",")
              : [];

            for (let i = 0; i < urlList.length; i++) {
              this.sucaiPackList.push({
                ccSupportMaterialUrl: urlList[i],
                ccSupportMaterialTime: ccSupportMaterialTimeList[i],
                ccSupportMaterialName: ccSupportMaterialNameList[i],
                ccSupportMaterialSize: ccSupportMaterialSizeList[i]
              });
            }
            console.log(urlList);
            ...
            this.courseName = ret.data.ccourseName;
            this.rightTitle = ret.data.ccourseName;
            this.setIsEdit(ret.data.catList);
            this.catalogList = ret.data.catList;
    ...
    setIsEdit(data) {
      data.forEach(element => {
        ...
        if (element.sonCatList.length > 0) {
          this.setIsEdit(element.sonCatList);
        }
      });
    },
    // 点击课程名
    ...
    handleNodeClick(bank) {
      ...
      this.rightTitle = bank.catalogName;
          ...
          if (ret.code === 200) {
            if (catalogParentId == 0) {
              this.catalogList.push({
                catalogId: ret.data.catalogId,
                catalogName: catalogName,
                sonCatList: [],
                isEdit: false
              });
            } else {
              parentCatalog.sonCatList.push({
                catalogId: ret.data.catalogId,
                catalogName: catalogName,
                sonCatList: [],
                isEdit: false
    ...
    appendNode(node, data) {
      this.expendArr = [data.catalogId];
      let catalogName = "新建章节";
      let catalogParentId = data.catalogId;
      this.addCatalogCommon(catalogName, catalogParentId, data);
    },
    //修改章节
    editNode(node, data, ev) {
      this.oldEditCatalogCatalogName = data.catalogName;
      data.isEdit = true;
      this.$nextTick(() => {
        let inpt =
    ...
    removeNode(node, data) {
      this.$confirm(
        ...
        axios
          .request({
            method: "post",
            url: "/ccourse/v4/cCourseCatalog/removeCatalog",
            data: {
              catalogId: data.catalogId
            }
          })
          .then(res => {
            ...
            if (ret.code === 200) {
              ...
              const children = parent.data.sonCatList || parent.data;
              const index = children.findIndex(
                d => d.catalogId === data.catalogId
              );
              children.splice(index, 1);
              //如果删除的章节是最后一个章节或者是选中的章节  跳转到云课
              if (
                this.catalogList.length == 0 ||
                data.catalogId == this.currentCatalog.catalogId
              ) {
                this.clickCourse();
              }
              this.$message({
    ...
    loadCatalogList() {
      ...
      axios
        .request({
          method: "post",
          url: "v1/ccCat/selectLeafsOfCat",
          data: {
            catId: this.currentCatalog.catalogId
          }
        })
        .then(res => {
          let ret = res.data;
    ...
    editComonUploadResFile() {
      let data = {};
      data.catalogId = this.currentResId;
      data.catalogName = this.addResForm.title;
      data.catalogSubType = this.currentResData.catalogSubType;
      // data.needDel = 1
      //修改  重新上传
    ...
    editContent(row) {
      this.currentResId = "";
      this.addResForm.title = row.catalogName;
      this.clearFile();
      this.currentResId = row.catalogId;
    ...
    //修改测试任务 重新选题
    againSeletTest(row) {
      this.currentTestTaskTitle = row.catalogName;
      this.currentTestTaskId = row.catalogId;
      this.drawer2 = true;
    },
    //删除课程内容
    ...
    moveContent(row) {
      ...
      this.currentMoveId = row.catalogId;
      this.isMoveShow = true;
    },
    getMoveSelected(el) {
      this.moveToCatalog = el.catalogName;
    ...
    doMoveContent() {
      if (this.moveToCatalogId || this.moveToCatalogId === 0) {
        if (this.isCatalog) {
          if (this.moveToCatalogId != this.currentCatalog.catalogId) {
      ...
      this.$confirm("删除后不可恢复, 是否继续?", "提示", {
        ...
        .then(() => {
          axios
            .request({
              method: "post",
              url: "v1/cCourse/deleteCCMaterialFile",
              data: {
