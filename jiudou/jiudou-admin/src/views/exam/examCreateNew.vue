    ...
    <el-tabs class="exam-create-new" v-model="current" @tab-click="handleClick">
      <el-tab-pane class="system container" label="考试信息" name="first">
                ...
                >
                  ...
                  <el-radio label="1"
                    >有限次<el-input
                      ...
                      placeholder="次数"
                      size="small"
                      :disabled="form.answerTimesType == 0"
                ...
                >
                  <el-radio label="1"
                    ><el-input
                      ...
                      placeholder="及格分数"
                      size="small"
                      :disabled="form.standardScoreType == 0"
                  ...
                  <ul style="padding-top:5px">
                    <li style="margin-bottom: 20px;">
                      <el-radio label="1"
                        ...
                        <el-select
                          ...
                          :disabled="payStatus == 3 || form.examType != 1"
                    ...
                    <li style="margin-bottom: 20px;">
                      <el-radio label="2"
                        ...
                        <el-input
                          ...
                          :disabled="form.examType != 2 || payStatus == 3"
                          oninput="value=value.replace(/[^0-9]/g,'')"
                        ></el-input>
                        <!-- <el-button
                          size="small"
                    ...
                    <li style="margin-bottom: 20px;">
                      <el-radio label="3"
                        >按名单考试
                        <el-button
                          v-if="form.examType == 3 && !nameListClass"
                          size="small"
        ...
        <div style="display:flex;justify-content:center;margin-top:100px">
          <div>
            <el-button size="medium" type="primary" @click="doSaveExam"
              >保存</el-button
            >
          ...
          <el-container>
            <el-aside width="310px">
              <div class="loadWord-btn">
                <el-button
                  ...
                  >清空试题</el-button
                >
                <el-button size="small" type="primary" @click="addFixTimu"
                  >+固定题</el-button
                >
                <el-button size="small" type="primary" @click="addRandomRule"
                  >+随机策略</el-button
                >
              </div>

              <!-- <el-button type="primary" class="loadWord" @click="goPublish"
                >下一步</el-button
              > -->
            ...
            <el-main class="timuBox">
              ...
              >
                ...
                <div>
                  <el-button size="medium" type="primary" @click="addFixTimu"
                    >+固定题</el-button
                  >
                  <el-button size="medium" type="primary" @click="addRandomRule"
                    >+随机策略</el-button
                  >
                </div>
              </div>
            ...
            <div class="bottom-f">
              <el-form>
                ...
                <el-form-item label="摄像头开启抓拍">
                  <el-switch
                    v-model="form.isCamera"
                    @change="changeIsCamera"
                  ></el-switch>
        ...
        <div style="display:flex;justify-content:center;margin-top:100px">
          <div>
            <el-button size="medium" type="primary" @click="doSaveExam"
              >保存</el-button
            >
            <el-button size="medium" type="primary" @click="doPublish"
              >发布考试</el-button
            >
          </div>
        </div>
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button size="small" type="primary" @click="delRandomRule"
            >删除策略</el-button
          >
        </span>
        <!-- <el-scrollbar height="300px"> </el-scrollbar> -->
...
export default {
  ...
  data() {
    return {
      ...
      current: "first",
      payStatus: 0,
      isShowNameImport: false, // 名单考试是否显示导入名单
      form: {
  ...
  methods: {
            ...
            if (res.data.unUseBankFlag) {
              this.$alert(
                ...
                "",
                {
                  ...
                  center: true,
                  callback: action => {
                    let ele = document.querySelector(".v-modal");
    ...
    importNameList() {
      ...
      axios
        .request({
          method: "post",
          url: `/clazz/clazz/member/batchImportExcelData`,
          data: {
            classId: this.nameListClass,
            memberListJson: JSON.stringify(this.nameList)
          }
        })
        .then(res => {
          let ret = res.data;
          if (ret.code === 200) {
            ...
            if (this.showImport2) {
              ...
              this.examStuPage.currentPage = 1;
              this.getExamStuList();
            }
          } else {
            this.$message({
    ...
    delExamStu(row) {
      axios
        .request({
          method: "post",
          url: "/clazz/clazz/member/kickOut",
          data: {
            cmId: row.id
          }
        })
        ...
        .then(res => {
          ...
          if (ret.code === 200) {
            ...
            ret.data.content.forEach(element => {
              subjectIds.push(element.bankId);
            });
    ...
    rePartIn() {
      this.showImport2 = true;
      this.$nextTick(function() {
        this.$refs.subjectsFile2.clearFile();
      });
    },
    downloadFormwork() {
      // 导出模板
      let url =
        "https://find-1253402545.cos.ap-beijing.myqcloud.com/%E8%80%83%E8%AF%95%E5%90%8D%E5%8D%95%E5%AF%BC%E5%85%A5%E6%A8%A1%E6%9D%BF.xls";
    ...
    doImportSure() {
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          ...
          fd.append("classId", this.nameListClass);
          ...
          axios
            .request({
              method: "post",
              url: "/clazz/clazz/member/parseExcelData",
              data: fd
            })
            .then(
              function(data) {
    ...
    doImportSure2() {
      this.$refs["ruleForm2"].validate(valid => {
        if (valid) {
          ...
          if (this.uploadFile2.type != "application/vnd.ms-excel") {
            this.$message({
              message: "名单文件不是Excel格式，请检查",
              type: "warning"
            });
          ...
          // this.isNameListShow = true;
          let fd = new FormData();
          fd.append("file", this.uploadFile2);
          fd.append("classId", this.nameListClass);
          // fd.append("mhId", this.id);
          // fd.append("school", this.ruleForm.partInSchool);
          this.loading.nameListLoading = true;
    ...
    getFee(classId) {
      ...
      if (this.form.examType == 2) {
        data.examNumber = this.form.examNum;
      }
      if (this.form.examType == 1 || this.form.examType == 3) {
      ...
      axios
        .request({
          ...
          url: "/exam/exam/config/getCharge",
          data
        })
        .then(res => {
          let ret = res.data;
    ...
    goPay() {
      ...
      if (examType == 1) {
        ...
        data.classId = this.nameListClass;
      }
      if (examType == 2) {
      ...
      if (isCamera) {
        if (gapTime <= 0 || gapTime == "") {
          this.$message({
            type: "warning",
            message: "请设置摄像头抓拍间隔时间"
          });
          return;
        }
      }
        ...
        if (valid) {
          axios
            ...
            .then(res => {
              ...
              if (ret.code == 200) {
                var CountEvent = window.JAnalyticsInterface.Event.CountEvent;
                var cEvent = new CountEvent("create_randomRule");
                window.JAnalyticsInterface.onEvent(cEvent);
      ...
      axios
        ...
        .then(res => {
          ...
          if (ret.code === 200) {
            ...
            if (type == 1 && ret.data.classId) {
              this.getClassStuNum2(ret.data.classId);
            }
            if (type == 3 && !ret.data.classId) {
              this.isShowNameImport = true;
            } else {
              this.isShowNameImport = false;
            }
            ...
            this.form.examNum = examNumber || ""; // 扫码考试考试人数
            // 考试时间
      ...
      axios
        ...
        .then(res => {
          ...
          if (ret.code === 200) {
            ...
            //       element.originScore = element.score;
            //       if (element.sonSubList && element.sonSubList.length > 0) {
