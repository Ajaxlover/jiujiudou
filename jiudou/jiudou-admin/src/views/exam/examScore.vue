<template>
  <el-container class="exam-score">
    <el-header style="height: 50px">
      <span @click="toExamCheck" style="cursor:pointer;color:#2BA1F4;"
        >成绩批改</span
      >
      <i class="el-icon-arrow-right"></i> <span>试卷批阅</span>
    </el-header>
    <el-container>
      <el-aside width="310px">
        <!-- <el-button type="primary" @click="toPublishExam" class="loadWord"
          >去发布</el-button
        > -->
      ...
      <div class="rightBox">
        <div class="rightBox-t">
          <div>学生姓名</div>
          <div>{{ stuInfo.stuName ? stuInfo.stuName : "无" }}</div>
          <div>考试成绩</div>
          <div>{{ stuInfo.userScore ? stuInfo.userScore : 0 }}</div>
          <div>考试状态</div>
          <div :class="[stuInfo.passFlag == 1 ? 'r-active' : 'w-active']">
            {{ stuInfo.passFlag == 1 ? "通过" : "未通过" }}
          </div>
        </div>
        ...
        <!-- <div class="rightBox-b">
          <el-button type="primary">提交批阅</el-button>
          <el-button icon="el-icon-download" @click="exportStuExam" type="plain"
            >答卷导出</el-button
          >
        </div> -->
      </div>
  ...
  methods: {
      ...
      this.$refs["scoreForm"].validate(valid => {
        if (valid) {
          ...
          if (!this.notPic) {
                  ...
                  axios
                    ...
                    .then(res => {
                      ...
                      if (res.status == 200) {
                        ...
                        let picJsonArr = [
                          {
                            bankId: this.pTimu.bankId,
                            parentId: this.pTimu.parentId,
                            score: this.formLabelAlign.score,
                            remark: this.formLabelAlign.comment,
                            answerRemark: accessUrl
                          }
                        ];
                        const picData = new FormData();
                        picData.append(
                          "reviewListJson",
                          JSON.stringify(picJsonArr)
                        );
                        picData.append(
                          "examResultId",
                          this.examInfo.examResultId
                        );
            ...
            data.append("reviewListJson", JSON.stringify(notPicJsonArr));
            data.append("examResultId", this.examInfo.examResultId);
      ...
      axios
        ...
        .then(res => {
          ...
          if (ret.code == 200) {
            ...
            this.btnLoading = false; // 关闭按钮loading
            this.notPic = false;
            this.type = 0;
            this.getTimuList();
          } else {
    ...
    getTimuDetail() {
      axios
        .request({
          method: "post",
          url: "/exam/exam/result/getDetail",
          data: {
        ...
        .then(res => {
          ...
          if (ret.code == 200) {
            ...
            this.stuInfo.stuName = stuName;
            this.stuInfo.userScore = userScore;
            this.stuInfo.passFlag = passFlag;
            this.examInfo.examName = examName;
            this.examInfo.examResultId = examResultId;
