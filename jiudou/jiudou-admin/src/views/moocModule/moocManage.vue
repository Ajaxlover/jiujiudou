          ...
          <el-table-column label="操作" align="center" width="150">
            <template slot-scope="scope">
              ...
              <el-button
                type="text"
                size="small"
                @click="moocEditStatus(scope.row)"
                style="margin-left:0"
                >{{ moocStatusWay(scope.row.status + 1) }}</el-button
              >
              <el-button
                type="text"
                size="small"
                @click="openMoocBindClass(scope.row)"
                style="margin-left:0"
                >绑定</el-button
              >
              <el-button
    ...
    >
      ...
      <el-checkbox-group v-model="bindValue" style="padding-left:90px;">
        <el-checkbox
          ...
          :disabled="item.moccId && currentCourse.id != item.moccId"
          >{{ item.clName }}</el-checkbox
        >
      </el-checkbox-group>
      <div class="inputTips" style="text-align:center;margin-top:30px">
        每个班级限1个慕课，当班级已绑定其他慕课时需先解除绑定
      </div>
      <span slot="footer">
        <el-button size="small" type="primary" @click="bindSure"
          >确定</el-button
        >
        <!-- isBindShow = false -->
        <el-button size="small" @click="delBind">解绑</el-button>
      </span>
    </el-dialog>
    <!-- 调价 -->
    <el-dialog
  ...
  methods: {
    ...
    bindSure() {
      axios
        .request({
          method: "post",
          url: "/openccourse/openCcourse/bindClass",
          data: {
            moccId: this.currentCourse.id,
            classIds: this.bindValue.join(",")
          }
        })
        .then(res => {
          let ret = res.data;
