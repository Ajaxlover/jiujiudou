    ...
    <el-main>
      ...
      <ul class="countTop">
        <li>
          ...
          <div>{{stuStaticData.studyToday}}</div>
        </li>
        <li>
          <div>
            <span class="block"></span>
          ...
          <div>{{stuStaticData.studySpeed}}</div>
        </li>
        <li>
          <div>
            <span class="block"></span>
          ...
          <div>{{stuStaticData.studyEnd}}</div>
        </li>
      </ul>
      <div class="countTitle">学员名单<el-button size="small" style="float:right" type="primary" @click="improt">导出</el-button></div>
      ...
      :header-cell-style="{'background-image':'linear-gradient(-180deg, #F2F3F7 0%, #DEE5ED 100%)','color':'#333333','height':'40px','padding':'0'}">
        ...
        <el-table-column label="已学" align="center">
          <div slot-scope="scope" class="info">{{scope.row.studySpeed}}</div>
        </el-table-column>
...
export default {
  ...
  mounted() {
    this.bus.$on('coursename', function(coursename) {
  ...
  methods: {
    ...
    loadStuStatic() {
      axios.request({
        method: 'post',
        url: '/openccourse/openCcourseDone/list',
        data: {
    ...
    loadStuList(id) {
      axios.request({
        ...
        let ret = res.data
        if (ret.code === 200) {
          this.stuListData = ret.data.records.person
          this.stuStaticData = ret.data.records
          if(this.stuStaticData.singleTotalNum * this.stuStaticData.stuNum == 0) {
            this.totalProcess = 0 + '%'
          }else {
            let p = Math.round(this.stuStaticData.totalDoneNum / (this.stuStaticData.singleTotalNum * this.stuStaticData.stuNum) * 100)
    ...
    improt() {
      for (let i = 0; i < this.stuListData.length; i++) {
        this.stuListData[i].number = i + 1
      }
      ...
      form.setAttribute('action', process.env.BASE_URL + "/openccourse/openCcourseDone/export")
      ...
      input2.value = JSON.stringify(this.stuListData)
      let input3 = window.document.createElement("input")
      input3.type = 'hidden'
      input3.name = 'name'
      input3.value = this.courseName
