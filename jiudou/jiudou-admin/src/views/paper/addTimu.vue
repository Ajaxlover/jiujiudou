<template>
  <el-container class="addTimu">
    <el-header height="50px">
      <span class="" @click="toPaper">试卷管理</span>
      <i class="el-icon-arrow-right"></i> <span>添加试题</span>
    </el-header>
    <el-main>
      ...
      <el-form :inline="true" v-model="testSelectForm" size="small">
        ...
        <el-form-item v-if="testSelectForm.rlType == 1" style="width:200px;">
          <el-cascader
...
export default {
  ...
  methods: {
    ...
    SelectionChange() {
      this.$nextTick(() => {
        let table = this.testSelectList;
        table.forEach(row => {
          if (row.isSelected == 1) {
            this.$refs.testSelectListTable.toggleRowSelection(row, true);
          }
        });
      });
    },
    ...
    testComTkSearch() {
      if (this.testSelectForm.comLib) {
        this.checkLib(this.testSelectForm.comLib);
    ...
    //点击我的库查询知识点
    testMyTkSearch() {
      this.checkLib(this.testSelectForm.myLib);
      this.loadMyLibTip();
      this.testSelectPage.page = 1;
      this.loadSubjects();
    },
